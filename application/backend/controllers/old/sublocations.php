...
class sublocations extends CI_Controller {
	...
	public function __construct(){
        ...
        public function index(){
				...
				$data["master_body"]="sublocations";  
                $data["singular"]="sublocations";  
                $data["nav"] = "locations";
                $data["main_nav"] = "jobs";
				 $data["sub_nav"] = "sublocations";
                $data['classifications']= $this->sublocation_model->get_items();
        ...
        public function deteteitem()
        {
            if(isset($_GET['deleteitem'])and!empty($_GET['deleteitem']))
            {
                ...
                $result = $this->sublocation_model->deleteItem($itemid);
                if($result==1)
                {
		...
		if ($this->form_validation->run() == FALSE)
		{
			$data['valerror']=1;
                ...
                if($errorflag==0)
                {
                     ...
                     $status = $this->input->post('status');
                     $cat = $this->sublocation_model->get_item_by_name($name, $parent_id);
                     if(count($cat)<1)
                     {
                     $insert_id =  $this->sublocation_model->insert_item($name,$parent_id,$status);
                        if($insert_id)
                        {
                            $data['sucess']= 1;
                        }
                ...
                $data['classifications']= $this->location_model->get_items($parent);
		$this->load->theme('adminlayout',$data);
        }
        public function sub_location($parent=NULL){
            ...
            if(empty($parent))
                ...
                $parent_name= $this->sublocation_model->get_item($parent);
                if(count($parent_name)==1)
                {
                    $data["sub_nav"] = $parent_name[0]->name;
                    $data["main_nav"] = "jobs";
                    $data['classifications']= $this->sublocation_model->get_items($parent);
                    $this->load->theme('adminlayout',$data);
                    return;
                }
                else
		...
		if ($this->form_validation->run() == FALSE)
		{
			$data['valerror']=1;
                ...
                if($errorflag==0)
                {
                     ...
                     $insert_id =  $this->sublocation_model->update_item($name,$parent_id,$status,$oldid);
                        if($insert_id)
                        {
                            $data['sucess']= 1;
                        }
		...
		$data["master_body"]="edititem";  
                ...
                $parent = 0;
                $data['classifications']= $this->sublocation_model->get_items($parent);
                $data['item'] = $this->sublocation_model->get_item($id);
